#!/usr/bin/env python3
# text_ocr_doc_classifier_gemini_💬.py
# Purpose: Classify documents using Google Gemini 1.5 Flash text capabilities by extracting text from PDF pages
...
def load_environment():
    ...
    for key in os.environ:
        if key.startswith(("AZURE_", "GOOGLE_")):
            logger.info(f"✅ Set environment variable: {key}")

class GeminiTextWrapper:
    """Wrapper for Google Gemini API with text capabilities"""
    ...
    def __init__(
        ...
        """Initialize the Google Gemini wrapper with text capabilities
    ...
    def text_chat_completion(self, messages, max_retries=2, **kwargs):
        """Make a chat completion request with text
        ...
        while retries <= max_retries:
            try:
                ...
                print(f"🚀 Making Gemini Text API call{attempt_str}")
                ...
                estimated_input_tokens = len(text_prompt) // 4
                estimated_output_tokens = len(response.text) // 4
                
                # Calculate cost (approximate)
                # Gemini 1.5 Flash pricing: $0.000035 / 1K input tokens, $0.000070 / 1K output tokens
                ...
                error_msg = str(e)
                print(f"❌ Gemini Text API call failed: {error_msg}")
...
def extract_text_from_pdf_pages(pdf_path, max_pages=None):
    """Extract text from the first N pages of a PDF
    
    Args:
        pdf_path (str): Path to the PDF file
        max_pages (int, optional): Maximum number of pages to extract. Defaults to None (uses PAGES_NO env var).
    ...
    try:
        ...
        with fitz.open(pdf_path) as doc:
            ...
            logger.info(f"📄 Extracting text from {num_pages} pages of PDF with {len(doc)} total pages")
            ...
            for page_num in range(num_pages):
                try:
                    ...
                    # Log success or warning based on content
                    if page_text.strip():
                        logger.info(f"✅ Successfully extracted text from page {page_num+1} ({len(page_text)} characters)")
                    else:
            ...
            if text.strip():
                ...
                logger.warning(f"⚠️ No text could be extracted from the PDF")
...
def classify_document_with_text(file_path, prompt_template, api):
    """Classify a document using text extraction
    
    Args:
        file_path (str): Path to the document
        prompt_template (str): Prompt template for classification
        api (GeminiTextWrapper): API wrapper
    ...
    try:
        ...
        # Prepare messages with extracted text
        messages = [
            {"role": "system", "content": prompt_template},
            {"role": "user", "content": f"Please analyze this document from {file_path}:\n\n{extracted_text}"}
        ]
        ...
        # Make API call
        logger.info(f"🔍 Sending extracted text ({len(extracted_text)} characters) to Gemini Text API")
        response = api.text_chat_completion(messages, max_retries=2, max_tokens=max_tokens)
...
def test_documents_with_text():
    """Test document classification using text extraction"""
    # Load environment variables
    load_environment()
    ...
    if model in ["gpt-4o", "gpt-4o-mini"]:
        # Use existing Azure OpenAI implementation
        from text_ocr_doc_classifier import test_documents_with_text as azure_test
        return azure_test()
        ...
        api = GeminiTextWrapper(budget_limit=10.0, timeout=120, model_name=model)
        ...
        for idx, (_, row) in enumerate(validation_data.iterrows(), 1):
            ...
            # Classify document
            result = classify_document_with_text(file_path, prompt_template, api)
        ...
        avg_text_length = total_text_length / len(results) if results else 0
        ...
        results_filename = f"gemini_text_classification_results_{timestamp}.json"
        run_summary_filename = f"gemini_text_run_summary_{timestamp}.json"
        ...
        print("\n" + "="*70)
        print(f"🏆 GEMINI TEXT CLASSIFICATION RESULTS SUMMARY 💬")
        print("="*70)
        ...
        print("\n📋 DETAILED RESULTS:")
        print(f"{'FILENAME':<40} | {'PREDICTED':<20} | {'EXPECTED':<20} | {'CORRECT':<10} | {'TIME (s)':<10} | {'COST ($)':<10} | {'TEXT LEN':<10}")
        print("-"*130)
        
        for detail in detailed_results:
            correct_mark = "✅" if detail['correct'] else "❌"
            print(f"{detail['file']:<40} | {detail['predicted']:<20} | {detail['expected']:<20} | {correct_mark:<10} | {detail['response_time']:.2f} | ${detail['cost']:.6f} | {detail['text_length']}")
