#!/usr/bin/env python3
# text_ocr_doc_classifier.py
# Purpose: Classify documents using text/OCR extraction from the first 2 pages
...
class AzureOpenAITextWrapper:
    """Wrapper for Azure OpenAI API with text capabilities"""
    ...
    def __init__(
        ...
        """Initialize the Azure OpenAI wrapper with text capabilities
    ...
    def text_chat_completion(self, messages, max_retries=2, **kwargs):
        ...
        while retries <= max_retries:
            try:
                ...
                print(f"🚀 Making Text API call{attempt_str} to: {self.api_base}/openai/deployments/{self.deployment}/chat/completions")
                ...
                if response.status_code == 200:
                    ...
                    print(f"❌ Text API call failed: {error_msg}")
                ...
                print(f"⏱️ ❌ Text API call failed: {error_msg}")
...
def classify_document_with_text(file_path, prompt_template, api):
    ...
    Args:
        ...
        api (AzureOpenAITextWrapper): API wrapper
    ...
    try:
        ...
        # Make API call
        logger.info(f"🔍 Sending extracted text ({len(extracted_text)} characters) to text API")
...
def test_documents_with_text():
    ...
    api = AzureOpenAITextWrapper(budget_limit=10.0, timeout=120)
    ...
    results_filename = f"text_ocr_classification_results_{model_name}_{timestamp}.json"
    run_summary_filename = f"text_ocr_run_summary_{model_name}_{timestamp}.json"
    ...
    print("\n" + "="*70)
    print(f"🏆 TEXT OCR CLASSIFICATION RESULTS SUMMARY ({model_name})")
    print("="*70)
    ...
    for detail in detailed_results:
        ...
        print(f"{detail['file']:<40} | {detail['predicted']:<20} | {detail['expected']:<20} | {correct_mark:<10} | {detail['response_time']:.2f} | ${detail['cost']:.4f} | {detail['text_length']}")
