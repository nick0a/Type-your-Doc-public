...
async function evaluateDocument(documentIndex) {
  try {
    ...
    // Filter for documents that have valid SOF pages
    const sofDocuments = validDocuments.filter(docName => {
      const pages = validationDataset[docName];
      // Check if any page is a Statement of Facts page
      return pages.some(page => {
        return page.subcategory.toLowerCase().includes('statement of facts') ||
               page.subcategory.toLowerCase().includes('sof');
      });
    });
    
    if (sofDocuments.length === 0) {
      console.error('Error: No Statement of Facts documents found in validation dataset');
      process.exit(1);
    }
    
    console.log(`Found ${sofDocuments.length} SOF documents in validation dataset`);
    
    // Select a document - either from index or random
    let selectedDocumentFilename;
    if (documentIndex !== undefined && documentIndex < sofDocuments.length) {
      selectedDocumentFilename = sofDocuments[documentIndex];
    } else {
      // Random selection
      const randomIndex = Math.floor(Math.random() * sofDocuments.length);
      selectedDocumentFilename = sofDocuments[randomIndex];
    }
